...
export default function Checkout() {
  ...
  /* ─── Redirect if cart is empty (only before order is placed) ─── */
  useEffect(() => {
    // Don't redirect if order was just placed — cart will be cleared intentionally
    if (!orderPlaced && (!cartData || cartData.length === 0)) {
      router.push('/user/cart')
    }
  }, [cartData, router, orderPlaced])
  ...
  /* ── Helper: mark order placed AND clear cart ── */
  const completeOrder = (total: number) => {
    setPaidTotal(total)
  ...
  const openRazorpay = async () => {
    ...
    const options = {
      ...
      // 3. On success → verify on backend, then clear cart
      handler: async (response: {
        ...
        try {
          ...
          if (verifyRes.data.success) {
            completeOrder(finalTotal)   // ← clears cart + shows success
          } else {
