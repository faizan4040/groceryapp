  ...
  if (orderPlaced) {
    return (
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        className="min-h-screen flex items-center justify-center bg-linear-to-br from-emerald-50 via-white to-teal-50"
      >
        <div className="text-center space-y-6 p-10 max-w-md">
          <motion.div
            ...
            className="w-28 h-28 bg-emerald-100 rounded-full flex items-center justify-center mx-auto"
          >
          ...
          >
            {paymentMethod === 'cod'
              ? `Your order is placed. Pay ${fmt(paidTotal)} on delivery.`
              : `Payment of ${fmt(paidTotal)} received. Your order is on its way! 🚀`}
          </motion.p>
          ...
          >
            <button
              ...
              className="px-6 py-3 border-2 border-emerald-600 text-emerald-600 rounded-2xl font-semibold hover:bg-emerald-50 transition-all"
            >
              Track Order
            </button>
            <button
              onClick={() => router.push('/')}
              className="px-6 py-3 bg-emerald-600 text-white rounded-2xl font-semibold hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-200"
            >
              Shop More
            </button>
          </motion.div>
            ...
            <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
              ...
              <div className="p-6 space-y-5">
                ...
                <button
                  onClick={handleOrder}
                  disabled={isLoading}
                  className="w-full py-4 rounded-2xl font-bold text-white text-base bg-linear-to-r from-emerald-600 to-teal-500 hover:from-emerald-700 hover:to-teal-600 active:scale-[0.98] transition-all shadow-lg shadow-emerald-200 disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {isLoading ? (
...
// const BADGE_ACTIVE: Record<string, string> = {
//   emerald: 'bg-emerald-100 text-emerald-700',
//   violet:  'bg-violet-100 text-violet-700',
...
//   /* ─── Address ─── */
//   const [address, setAddress] = useState<AddressState>({
//     fullName: '', mobile: '', city: '', state: '',
//     pincode: '', fullAddress: '', searchBar: '',
//   })
...
//         setLocationError('Could not detect location. Search or pin manually.')
//         setPosition([20.5937, 78.9629])
//       },
//       { enableHighAccuracy: true, maximumAge: 0, timeout: 10000 }
//     )
...
//     setAddress((prev) => ({ ...prev, searchBar: val }))
//     setSuggestions([])
...
//     const val = address.searchBar.trim()
//     if (!val) return
//     setIsSearching(true)
//     setSuggestions([])
//     try {
...
//       } else {
//         setLocationError('No results found. Try a different search.')
//       }
//     } catch (e) {
//       console.error('Search error:', e)
//     } finally {
...
//     setAddress((prev) => ({ ...prev, searchBar: s.display_name, fullAddress: s.display_name }))
//     setSuggestions([])
//     setLocationError('')
//   }

//   /* ─── Coupon logic ─── */
//   const VALID_COUPONS: Record<string, number> = { SAVE50: 50, FIRST100: 100, FRESH20: 20 }
//   const discount = couponApplied && VALID_COUPONS[coupon.toUpperCase()]
//     ? VALID_COUPONS[coupon.toUpperCase()]
//     : 0
...
//     } else {
//       setCouponApplied(false)
//       setCouponError('Invalid coupon code. Try SAVE50, FIRST100 or FRESH20.')
//     }
//   }
...
//   const handleOrder = async () => {
//     if (!address.fullName || !address.mobile || !address.fullAddress) {
//       alert('Please fill Name, Mobile, and Full Address.')
//       return
//     }
...
//   /* ─── Guards ─── */
//   if (cartData.length === 0) { router.push('/user/cart'); return null }
...
//         className="min-h-screen flex items-center justify-center bg-linear-to-br from-emerald-50 via-white to-teal-50"
//       >
//         <div className="text-center space-y-5 p-10 max-w-md">
//           <motion.div
//             initial={{ scale: 0 }}
//             animate={{ scale: 1 }}
//             transition={{ type: 'spring', stiffness: 200, damping: 12, delay: 0.2 }}
...
//           >
//             {paymentMethod === 'cod'
//               ? `Your order is placed. Pay ${fmt(total)} on delivery.`
//               : `Payment of ${fmt(total)} received. Your order is on its way!`}
//           </motion.p>
//           <motion.div
...
//             <button
//               onClick={() => router.push('/user/orders')}
//               className="px-6 py-3 border-2 border-emerald-600 text-emerald-600 rounded-2xl font-semibold hover:bg-emerald-50 transition"
//             >
//               Track Order
//             </button>
//             <button
//               onClick={() => router.push('/')}
//               className="px-6 py-3 bg-emerald-600 text-white rounded-2xl font-semibold hover:bg-emerald-700 transition shadow-lg shadow-emerald-200"
//             >
//               Shop More
//             </button>
//           </motion.div>
...
//               <div className="p-6 space-y-4">
//                 <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
//                   <InputField icon={<FaUser />} value={address.fullName}
//                     onChange={(v) => setAddress({ ...address, fullName: v })}
//                     placeholder="Full Name *" />
//                   <InputField icon={<FaPhone />} value={address.mobile}
//                     onChange={(v) => setAddress({ ...address, mobile: v })}
//                     placeholder="Mobile Number *" type="tel" />
//                 </div>
//                 <InputField icon={<FaMapMarkerAlt />} value={address.fullAddress}
//                   onChange={(v) => setAddress({ ...address, fullAddress: v })}
//                   placeholder="Full Address (House / Flat / Street) *" />
//                 <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
//                   <InputField icon={<FaCity />}     value={address.city}
//                     onChange={(v) => setAddress({ ...address, city: v })}    placeholder="City" />
//                   <InputField icon={<FaGlobe />}    value={address.state}
//                     onChange={(v) => setAddress({ ...address, state: v })}   placeholder="State" />
//                   <InputField icon={<FaMailBulk />} value={address.pincode}
//                     onChange={(v) => setAddress({ ...address, pincode: v })} placeholder="Pincode" type="number" />
//                 </div>
//               </div>
//             </motion.div>

//             {/* ── Map & Location Search Card ── */}
//             <motion.div
//               initial={{ opacity: 0, y: 20 }}
//               animate={{ opacity: 1, y: 0 }}
//               transition={{ duration: 0.35, delay: 0.08 }}
...
//                 icon={<FaMapMarkerAlt />}
//                 title="Pin Your Location"
//                 sub="Search or drag the marker to your exact address"
//               />
//               <div className="p-6 space-y-4">
...
//                       <input
//                         value={address.searchBar}
//                         onChange={(e) => handleSearchChange(e.target.value)}
//                         onKeyDown={(e) => e.key === 'Enter' && handleSearchSubmit()}
//                         placeholder="Search city, area, street, landmark…"
//                         className="w-full pl-11 pr-4 h-12 rounded-2xl border border-gray-200 bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-teal-500 transition text-sm"
//                       />
//                     </div>
...
//                       disabled={isSearching || !address.searchBar.trim()}
//                       className="h-12 px-5 rounded-2xl bg-linear-to-r from-teal-600 to-cyan-500 text-white font-semibold text-sm flex items-center gap-2 hover:from-teal-700 hover:to-cyan-600 active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-md shadow-teal-100 shrink-0"
//                     >
//                       {isSearching
//                         ? <><FaSpinner className="animate-spin" /><span className="hidden sm:inline">Searching…</span></>
...
//                         animate={{ opacity: 1, y: 0 }}
//                         exit={{ opacity: 0, y: -6 }}
//                         className="absolute top-14 left-0 right-0 z-50 bg-white border border-gray-200 rounded-2xl shadow-xl overflow-hidden"
//                       >
//                         {suggestions.map((s) => (
//                           <button
//                             key={s.place_id}
//                             onClick={() => pickSuggestion(s)}
//                             className="w-full text-left px-4 py-3 text-sm text-gray-700 hover:bg-teal-50 hover:text-teal-700 transition border-b border-gray-50 last:border-0 flex items-start gap-2"
//                           >
//                             <FaMapMarkerAlt className="text-teal-500 mt-0.5 shrink-0 text-xs" />
...
//                     <FaMapMarkerAlt className="shrink-0" /> {locationError}
//                   </div>
//                 )}
...
//                 </div>
//                 <p className="text-xs text-gray-400 text-center">
//                   📍 Drag the green pin to fine-tune your delivery spot
//                 </p>
//               </div>
//             </motion.div>
...
//                       }`}
//                     >
//                       <span className={active ? ICON_ACTIVE[opt.color] : 'text-gray-400'}>
//                         {opt.icon}
//                       </span>
//                       <div className="flex-1">
//                         <p className="font-semibold text-sm text-gray-800 flex items-center gap-2">
...
//                           {opt.badge && (
//                             <span className={`text-[10px] px-2 py-0.5 rounded-full font-bold ${
//                               active ? BADGE_ACTIVE[opt.color] : 'bg-gray-100 text-gray-400'
//                             }`}>
//                               {opt.badge}
//                             </span>
//                           )}
...
//                       </div>
//                       <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${
//                         active ? DOT_ACTIVE[opt.color] : 'border-gray-300'
//                       }`}>
//                         {active && <div className="w-2 h-2 rounded-full bg-white" />}
//                       </div>
//                     </button>
...
//                             onChange={(v) => setCardDetails({ ...cardDetails, expiry: v })}
//                             placeholder="MM / YY"
//                             accentColor="blue"
//                           />
//                           <InputField
...
//                             onChange={(v) => setCardDetails({ ...cardDetails, cvv: v })}
//                             placeholder="CVV"
//                             type="password"
//                             accentColor="blue"
//                           />
//                         </div>
//                         <p className="text-xs text-gray-400 flex items-center gap-1 pl-1">
//                           <FaLock className="text-blue-400" />
//                           Your card data is 256-bit encrypted.
//                         </p>
//                       </div>
//                     </motion.div>
//                   )}
...
//                       className="overflow-hidden"
//                     >
//                       <div className="pt-3 bg-orange-50 rounded-2xl p-4 text-sm text-orange-700 flex items-start gap-3">
//                         <FaUniversity className="shrink-0 mt-0.5" />
//                         <span>
//                           You'll be redirected to your bank's secure portal after placing the order.
//                           Supports SBI, HDFC, ICICI, Axis, Kotak, and 50+ banks.
//                         </span>
//                       </div>
//                     </motion.div>
//                   )}
...
//                     <><FaSpinner className="animate-spin" /> Processing…</>
//                   ) : paymentMethod === 'cod' ? (
//                     <><FaCheckCircle /> Place Order · {fmt(total)}</>
//                   ) : (
//                     <><FaLock /> Pay Securely · {fmt(total)}</>
//                   )}
//                 </button>
...
//   return (
//     <div className="relative group">
//       <span className={`absolute left-4 top-1/2 -translate-y-1/2 text-gray-400 text-sm transition-colors ${iconClass}`}>
//         {icon}
//       </span>
//       <input
//         type={type}
