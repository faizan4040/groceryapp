...
interface Notification {
  ...
  icon_bg: string;  // tailwind bg colour class
}
...
export function TopBar() {
  ...
  useEffect(() => {
    const fetchNotifications = async () => {
      try {
        ...
        /* ── 1. Low-stock / out-of-stock alerts from grocery DB ── */
        const groceryRes = await axios.get("/api/admin/stock-orders");
        const items: any[] = groceryRes.data.items ?? groceryRes.data.groceries ?? [];
        ...
        items
          ...
          .forEach((i) => {
            combined.push({
              ...
              msg:     i.stock === 0
                         ? `Out of stock: ${i.name}`
                         : `Low stock: ${i.name} — only ${i.stock} left`,
              time:    i.updatedAt ?? new Date().toISOString(),
              dot:     i.stock === 0 ? "bg-red-500"  : "bg-amber-400",
              icon_bg: i.stock === 0 ? "bg-red-50"   : "bg-amber-50",
            });
          });

        /* ── 2. Recent orders from orders DB ── */
        const ordersRes = await axios.get("/api/admin/get-orders?limit=5&sort=recent");
        const orders: any[] = ordersRes.data.orders ?? [];

        orders.forEach((o) => {
          combined.push({
            id:      `order-${o._id}`,
            msg:     `New order #${o.orderNumber ?? o._id.toString().slice(-6).toUpperCase()} received`,
            time:    o.createdAt,
            dot:     "bg-green-500",
            icon_bg: "bg-green-50",
          });
        ...
        /* ── 3. Sort newest-first, cap at 8 ── */
        combined.sort(
          (a, b) => new Date(b.time).getTime() - new Date(a.time).getTime()
        );

        setNotifs(combined.slice(0, 8));
      } catch (err) {
  ...
  /* ── "new" = arrived within last hour ── */
  const newCount = notifs.filter(
    (n) => Date.now() - new Date(n.time).getTime() < 60 * 60 * 1000
  ).length;
      ...
      <div className="flex items-center gap-2">
        ...
        <div ref={notifRef} className="relative">
          ...
          >
            <Bell size={16} />
            {newCount > 0 && (
              <span className="absolute -top-1 -right-1 min-w-4 h-4 bg-green-500 rounded-full ring-2 ring-white flex items-center justify-center">
                <span className="text-[9px] font-bold text-white px-0.5 leading-none">
          ...
          {notifOpen && (
            <div className="absolute right-0 top-12 w-80 bg-white border border-gray-100 rounded-2xl shadow-xl shadow-gray-200/80 overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200">
                  ...
                  {notifs.map((n) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-gray-700 group-hover:text-gray-900 transition-colors leading-snug">
                          {n.msg}
                        </p>
              ...
              {/* footer */}
              <div className="px-4 py-2.5 text-center bg-gray-50/60 border-t border-gray-100">
                <Link
                  href="/admin/notifications"
                  onClick={() => setNotifOpen(false)}
                  className="text-[11px] text-green-600 hover:text-green-700 font-semibold transition-colors"
                >
      ...
      <Sidebar className="bg-white! border-r! border-gray-100! w-60! flex flex-col shadow-sm **:data-sidebar:bg-white! [&_.sidebar-content]:bg-white!">
              ...
              <SidebarMenu className="gap-0.5">
                {adminSidebarMenu.map((item, idx) => {
                  ...
                  return (
                    <div
                      key={item.title}
                      className={`transition-all duration-300 ${mounted ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-3"}`}
                    ...
                    >
                      <SidebarMenuItem className="list-none">
                        <SidebarMenuButton
                          ...
                          className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm border cursor-pointer transition-all duration-200
                            ${highlighted
                          ...
                          {item.submenu ? (
                            <div className="flex items-center gap-3 w-full">
                              <div className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 transition-all duration-200
                                ${highlighted ? "bg-green-100 text-green-600" : "bg-gray-100 text-gray-400"}`}>
                                <item.icon size={14} />
                              </div>
                              <span className="flex-1 font-semibold text-[13px]">{item.title}</span>
                              <ChevronDown size={13} className={`transition-transform duration-300 ${isOpen ? "rotate-180 text-green-600" : "text-gray-400"}`} />
                            </div>
                          ) : (
                            <Link href={item.url} className="flex items-center gap-3 w-full">
                              ...
                              <span className="flex-1 font-semibold text-[13px]">{item.title}</span>
                              {isActive && <span className="w-1.5 h-1.5 rounded-full bg-green-500 shadow-sm" />}
                            </Link>
                          )}
                        </SidebarMenuButton>
                      </SidebarMenuItem>
                            ...
                            return (
                              <Link
                                ...
                                className={`flex items-center gap-2.5 text-[12.5px] px-3 py-2 rounded-lg transition-all duration-150
                                  ${isSubActive ? "bg-green-50 text-green-700 font-semibold" : "text-gray-400 hover:text-gray-700 hover:bg-gray-50"}`}
                              >
                                <span className={`w-1.5 h-1.5 rounded-full shrink-0 ${isSubActive ? "bg-green-500" : "bg-gray-300"}`} />
                                {sub.title}
                              </Link>
                            );
                          })}
        ...
        <div className="p-3 border-t border-gray-100 bg-gray-50/60">
          <div className="flex items-center gap-3 px-2 py-2.5 rounded-xl bg-white border border-gray-100 hover:border-gray-200 hover:shadow-sm transition-all duration-200">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-[12.5px] font-bold text-gray-800 leading-tight truncate">{user?.name || "Admin"}</p>
              <p className="text-[10px] text-gray-400 leading-tight truncate">{user?.email || "admin@email.com"}</p>
            </div>
            <button
              onClick={() => signOut({ callbackUrl: "/login" })}
              title="Sign out"
