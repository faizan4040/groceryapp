...
const EditRolemobile = () => {
  ...
  const { update }   = useSession();  // needed to refresh JWT token
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.data.success) {
        ...
        // CRITICAL: refresh the JWT so middleware sees the new role
        // Without this, middleware still reads role: null and loops back here
        await update({ role: selectedRole, mobile });

        toast.success("Profile updated!");

        // replace instead of push — can't go back to edit-role
    ...
    <div className="min-h-screen flex items-center justify-center bg-linear-to-br from-green-100 via-white to-yellow-100 p-6">
      ...
      >
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-10">
          {roles.map((r) => {
            ...
            return (
              ...
              >
                <div className={`w-14 h-14 mx-auto flex items-center justify-center rounded-full text-white mb-4 ${r.color}`}>
                  <Icon size={26} />
                </div>
        ...
        <motion.button
          ...
          className="w-full cursor-pointer bg-linear-to-r from-green-500 to-green-600 text-white py-4 rounded-xl text-lg font-semibold shadow-lg hover:shadow-xl transition disabled:opacity-60 disabled:cursor-not-allowed flex items-center justify-center gap-2"
        >
          {loading ? (
            <>
